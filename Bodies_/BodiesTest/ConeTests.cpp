...
{
	const double EXPECTED_VOLUME =  (M_PI * EXPECTED_RADIUS *
		EXPECTED_RADIUS * EXPECTED_HEIGHT / 3);
	BOOST_CHECK_EQUAL((int)cone.GetVolume(), (int)EXPECTED_VOLUME);
	CBody & coneAsBody = cone;
	BOOST_CHECK_EQUAL((int)coneAsBody.GetVolume(), (int)EXPECTED_VOLUME);
}
